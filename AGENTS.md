...
- 所有新增和修改的 `.c`、`.h` 文件必须使用 UTF-8 编码。
- 遇到无法应用局部补丁的文件时，先检查文件编码，不要反复尝试 `apply_patch`。
- 对旧 GBK 或 ANSI 文件，应先使用明确编码读取，再转换为 UTF-8。
- 禁止在乱码状态下覆盖保存文件。
- 修改完成后，使用严格 UTF-8 解码方式验证文件。
- 包含中文字符串常量的文件，转换编码后必须检查字符串内容是否保持正确。
