...
/**
 * @brief 配置 MPU6050 进入低功耗加速度计运动唤醒模式。
 *
 * 该接口用于 MCU 进入 Sleep 或 STOP 前保留 MPU6050 工作，并在检测到明显运动时通过
 * INT 引脚拉高唤醒 EXTI。配置后会关闭陀螺仪，只保留加速度计低频周期采样，以降低待机功耗。
 * 阈值和持续时间采用项目内保守默认值，后续可按实机效果再调。
 *
